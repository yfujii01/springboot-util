...
public class SecurityControllerTests extends AbstractControllerTests {
	...
	public void admin_page_access_ok_Test() throws Exception {
		...
		for (Object[] p : params) {
			...
			// page呼び出し
			MvcResult resule = mockMvc
					.perform((MockMvcRequestBuilders.get("/adminonly").header(SecurityConstants.HEADER_STRING, token))
							.with(SecurityMockMvcRequestPostProcessors.csrf()))
					.andExpect(MockMvcResultMatchers.status().isOk()).andReturn();
	...
	public void user_and_admin_page_access_ok_Test() throws Exception {
		...
		for (Object[] p : params) {
			...
			// page呼び出し
			MvcResult resule = mockMvc
					.perform((MockMvcRequestBuilders.get("/userandadminonly").header(SecurityConstants.HEADER_STRING, token))
							.with(SecurityMockMvcRequestPostProcessors.csrf()))
					.andExpect(MockMvcResultMatchers.status().isOk()).andReturn();
	...
	public void user_page_access_ok_Test() throws Exception {
		...
		for (Object[] p : params) {
			...
			// page呼び出し
			MvcResult resule = mockMvc
					.perform((MockMvcRequestBuilders.get("/useronly").header(SecurityConstants.HEADER_STRING, token))
							.with(SecurityMockMvcRequestPostProcessors.csrf()))
					.andExpect(MockMvcResultMatchers.status().isOk()).andReturn();
